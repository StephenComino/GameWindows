...
unsigned int initTexture(int myProgram) {
    ...
    unsigned char* data = stbi_load("C:\\Users\\scomi\\Desktop\\tex.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
    ...
    data = stbi_load("C:\\Users\\scomi\\Desktop\\tex3.jpg", &width, &height, &nrChannels, 0);
    if (data)
    {
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
        glGenerateMipmap(GL_TEXTURE_2D);
